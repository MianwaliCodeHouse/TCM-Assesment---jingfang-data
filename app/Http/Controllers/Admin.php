...
public function clinicanInfo(){
  return View("admin.admintop").View("admin/clinicanInfo");
}
public function updateClinicianInfo(Request $request){
  // Get the content from the form textarea
  $content = $request->input('content');

  // Write the updated content to clinicanInfo.html
file_put_contents(public_path('clinicanInfo.html'), $content);

  // Redirect back to the clinicanInfo page
  return redirect('/clinicanInfo')->with('success', 'Content updated successfully!');
}
...
public function notes()
{
  $ad=new AdminModel();
  $groups=$ad->getGroups();
  return View("admin.admintop").View("admin/notes",["groups"=>$groups]);
}
...
public function editNotes(Request $r)
{
  $name=$r->get("name");
  $threshold=$r->get("threshold");
  $id=$r->get("id");
  $admin=new AdminModel();
  $id=$admin->editNote($id,$name,$threshold);
  return "";
}
...
public function addInvite(Request $r)
{
  ...
  $country_licence_issued=$r->get("country_licence_issued");
  if($status=="enable")
  {
    $status="e";
  }
...
$d = Mail::send("admin.invitetemplate",$data,function($messages) use ($emailaddress){
$messages->to($emailaddress);
$messages->subject("Invitation to use the JingFang Assessment as a Clinican");
});

$ad=new AdminModel();
$ad->addInvite($firstname,$lastname,$emailaddress,$password,$status,$practice_address,$mailing_address,$phone_number,$licence_number,$country_licence_issued);
return "";
}
