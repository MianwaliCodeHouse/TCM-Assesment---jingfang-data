...
class AssessmentModel extends Model
{
    ...
    {
      $result=  DB::select(DB::raw("select * from invitations where clientid=$id and stage='Closed'"));
      return $result;
    }

    function getQuestions($examId=0)
    {
      if($examId==1) {
        return  DB::select(DB::raw("select questions.*,maingroup.name as maingroupname,maingroup.type from questions left join maingroup on questions.maingroupid = maingroup.id where questions.subgroupid=0 and maingroup.name NOT IN ('CCMQ','CCMQ-SF','BCQ_Negative','BCQ_Positive','BCQs') ;"));
      } else {
        return  DB::select(DB::raw("select questions.*,maingroup.name as maingroupname,maingroup.type from questions left join maingroup on questions.maingroupid = maingroup.id where questions.subgroupid=0 and questions.maingroupid=".$examId." ;"));  
      }
      
    //   $questions = GetQuestions::select('questions.*', 'maingroup.name as maingroupname', 'maingroup.type')
    // ->leftJoin('maingroup', 'questions.maingroupid', '=', 'maingroup.id')
    // ->where('questions.subgroupid', 0)
    // ->paginate(5);
    //   $result = $questions;
      return true;
    }
    ...
    invitations.maingroup_id,
    maingroup.name,
    invitations.attempt,
    invitations.stage,
    invitations.attemptdate,
    invitations.invitedate,
    invitations.testing,
    invitations.firstfamily, 
    clients.id AS clientid,
    clients.firstname,
    clients.lastname,
    clients.email
  FROM invitations
  INNER JOIN clients ON invitations.clientid = clients.id
  INNER JOIN maingroup ON maingroup.id = invitations.maingroup_id
  WHERE
    invitations.clientid = $clientid
    AND invitations.inviteby = $inviteby;"));
    return $result;
    }
    ...
    {
    // $result=  DB::select(DB::raw("select distinct invitations.clientid,invitations.maingroup_id from clinicans,invitations where clinicans.id=invitations.inviteby and invitations.inviteby=$id  order by invitations.id desc"));
    $result = DB::select(DB::raw("SELECT DISTINCT invitations.clientid, invitations.maingroup_id, invitations.id
FROM clinicans
JOIN invitations ON clinicans.id = invitations.inviteby
WHERE invitations.inviteby = $id
...
"));
    ...
    public function addInviteByUser($clientid,$random,$id,$date2,$testinguser,$maingroup)
    {

      $result=  DB::select(DB::raw("insert into invitations(clientid,invitationcode,attempt,stage,inviteby,invitedate,testing,maingroup_id) values($clientid,$random,'n','Not attempted',$id,'$date2',$testinguser,$maingroup)"));
    ...
    {
      $checkUser = DB::select(DB::raw("select * from clients where email='".$email."'"));
      if(!empty($checkUser)) {
