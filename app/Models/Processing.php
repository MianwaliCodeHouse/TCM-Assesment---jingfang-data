    ...
    foreach ($results as $r) {
      if ($r->subgroupid == 0) {
        ...
        if ($a != "") {
          ...
          else if ((int)$r->maingroupid < 18) {
            if ($r->questionanswer == $r->resultsanswer){
              $maingroupsCorrectQuestions[$a] += $r->weight;
            $answer = $mainGroupsAnsweredCorrect[$a];
            array_push($answer, $r->question . '***' . $r->priority);
            $mainGroupsAnsweredCorrect[$a] = $answer;
          ...
          else if((int)$r->maingroupid > 17 && (int)$r->maingroupid < 23){
            if($r->resultsanswer!='' && (int)$r->resultsanswer > 1){
              $maingroupsCorrectQuestions[$a] += $r->resultsanswer;
              $answer = $mainGroupsAnsweredCorrect[$a];
              array_push($answer, $r->question . '***' . $r->priority);
              $mainGroupsAnsweredCorrect[$a] = $answer;
            }else{
