...
<div class="container">
        ...
        <div class="col-sm-6">
            <table class="table table-bordered">
                <tr>
                    ...
                    <td style="color:green">{{ $firstname }} {{ $lastname }}</td>
                <tr>
                <tr>
                    <td style="font-weight:bold">Quiz Date</td>
                    <td style="color:blue">{{ $attemptdate }}</td>
                <tr>
            </table>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            <fieldset class="border rounded-3 p-3">
                <legend class="float-none w-auto px-3" style="font-weight:bold">Formula Families in Order of Scoring
                </legend>
                        ...
                        <table class="table mb-0 table-borderless table-striped small">
                            <thead>
                                <tr class="text-secondary">
                                    <th class="pt-4 pb-3 px-6">Family</th>
                                    <th class="pt-4 pb-3 px-6">Score</th>
                                    @if (
                                        $maingroups[0] == 'CCMQ' ||
                                            $maingroups[0] == 'CCMQ-SF' ||
                                            $maingroups[0] == 'BCQs' ||
                                            $maingroups[0] == 'BCQ_Negative' ||
                                            $maingroups[0] == 'BCQ_Positive')
                                    @else
                                        <th class="pt-4 pb-3 px-6">Priority Questions</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody id="tb">
                                ...
                                @for ($i = 0; $i < count($maingroups); $i++)
                                    <tr>
                                        ...
                                        @if (
                                            $maingroups[$i] == 'CCMQ' ||
                                                $maingroups[$i] == 'CCMQ-SF' ||
                                                $maingroups[$i] == 'BCQs' ||
                                                $maingroups[$i] == 'BCQ_Negative' ||
                                                $maingroups[$i] == 'BCQ_Positive')
                                            <td class="py-5 px-6">
                                                {{ count($maingroupsansweredcorrect[$i]) }}/{{ $maingroupstotalquestions[$i] }}
                                                -
                                                {{ number_format(((float) count($maingroupsansweredcorrect[$i]) / $maingroupstotalquestions[$i]) * 100, 2, '.', '') }}%
                                            </td>
                                        @else
                                            <td class="py-5 px-6">
                                                {{ $maingroupscorrectquestions[$i] }}/{{ $maingroupstotalquestions[$i] }}
                                                -
                                                {{ number_format(((float) $maingroupscorrectquestions[$i] / $maingroupstotalquestions[$i]) * 100, 2, '.', '') }}%
                                            </td>
                                            <td class="py-5 px-6"><?php if ($sgroups[$i] != '') {
                                                echo 'Yes';
                                            } else {
                                                echo 'No';
                                            } ?></td>
    ...
    @else
        <div class="row">
            ...
            <div class="col-sm-10">
                <fieldset class="border rounded-3 p-3">
                    <legend class="float-none w-auto px-3" style="font-weight:bold">Refinement Questions for Formula
                        Families that meet Priority or are over Threshold</legend>
                                ...
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingOne">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#collapseOneo{{ $i }}" aria-expanded="false"
                                            aria-controls="collapseOneo{{ $i }}">
                                            +
                                        </button>
                                    </h2>
                                    <div id="collapseOneo{{ $i }}" class="accordion-collapse collapse"
                                        aria-labelledby="headingOne"
                                        data-bs-parent="#accordionExampleo{{ $i }}">
                                        <div class="accordion-body">
                                            <span style="font-size:16px;font-weight:bold">Negative or N/A
                                                Answers</span><br>
