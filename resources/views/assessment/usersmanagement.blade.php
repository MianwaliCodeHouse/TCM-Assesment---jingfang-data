...
@foreach ($invites as $i)
          <?php
          $inv = $i->invites;
          if (!isset($currentUser[$i->clientid])) {
              $currentUser[$i->clientid] = $i;
          }
          ?>
@endforeach

@php  $invites = $currentUser;  @endphp
     ...
     <div class="modal-content">
        ...
        <div class="modal-body">
           ...
           <div class="mb-6">
              ...
              <input class="form-control" id="emailaddress" type="email" name="field-name"
                 placeholder="Enter Email Address">
           </div>
           <div class="mb-6">
              <label class="form-label" for="">Type</label>
              <select name="main_group" id="maingroup" class="form-select">
                 <option value="">Select Exam</option>
                 @foreach ($maingroups as $groups)
...
<div class="modal fade" id="inviteAgain" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
     <div class="modal-content">
        <div class="modal-header">
           <h5 class="modal-title" id="exampleModalLabel">Invite Again</h5>
           <button type="button" id="btnclose" class="btn-close" data-bs-dismiss="modal"
              aria-label="Close"></button>
        </div>
        <div class="modal-body">
           <input type="hidden" id="inviteAgainClientId">
           <div class="mb-6">
              <label class="form-label" for="">Type</label>
              <select name="main_group" id="inviteAgainMaingroup" class="form-select">
                 <option value="">Select Exam</option>
                 @foreach ($maingroups as $groups)
                    @if($groups->parent_id==0)
                    <option value="{{ $groups->id }}">{{ $groups->name }}</option>
        ...
        <div class="modal-footer">
           ...
           <button type="button"  class="btn btn-primary"><span
              id="whattodo" onclick="inviteagain(inviteAgainClientId.value,inviteAgainMaingroup.value)">Invite Again</span></button>
        </div>
     </div>
  </div>
</div>
     ...
     <div class="d-flex flex-wrap align-items-center justify-content-between">
        <div class="col-12 col-lg-auto mb-4 mb-lg-0">
           <div class="d-flex align-items-center">
              <h4 class="mb-0 me-2">Invites Users</h4>
...
<section class="py-8">
  ...
  <div id="tb">
     ...
     <?php
        $inv = $i->invites;
        if (!isset($currentUserId[$i->clientid])) {
            $currentUserId[$i->clientid] = $i->clientid;
        }
        ?>
     @if (count($inv) > 0)
     <?php $resultsavail = 0; ?>
     ...
     @endforeach
     <?php 
        if(!in_array($i->clientid,$currentUserId)){}
        ?>
     <div id="row">
           ...
           <div class="accordion-item">
              <h2 class="accordion-header" id="headingOne">
                 <button class="accordion-button" type="button" data-bs-toggle="collapse"
                    data-bs-target="#collapseOne{{ $i->clientid }}" aria-expanded="false"
                    aria-controls="collapseOne{{ $i->clientid }}">
                 &nbsp;{{ $inv[0]->firstname }} {{ $inv[0]->lastname }}</button>
              </h2>
              <div id="collapseOne{{ $i->clientid }}" style="background-color:white"
                    ...
                    <div class="row">
                       ...
                       <div class="col-lg-4">
                          <button class="btn btn-success"
                             onclick="inviteAgainClientId.value={{ $i->clientid }}" data-bs-toggle="modal" data-bs-target="#inviteAgain">Invite
                          Again</button>
                       </div>
                       @if ($resultsavail == 1)
                       <div class="col-12 d-flex justify-content-end mt-2">
                          <a href="<?php echo url('')?>/comparison?clientid={{ $i->clientid }}"
                             class='p-2' target="_blank">View Results Comparison</a>
                          ...
                          foreach ($inv as $item) {
                              if($item->stage=='Closed') {
                                $category = $item->name;
                                if (!isset($uniqueArray[$category])) {
                                    $uniqueArray[$category] = $item;
                                }
                              }
                          }
                          ...
                          @foreach ($uniqueArray as $iv)
                          @if (in_array($iv->name, $cat_names))
                          <a href="{{ route('assessment.graphs', ['name' => $iv->name, 'id' => $iv->maingroup_id, 'clientId' => $i->clientid]) }}"
                             class='p-2' target="_blank">View
                          {{ $iv->name }} Result </a>
                          {{-- @break --}}
                          @endif
                          @endforeach
                       ...
                       <table class="table mb-0 table-borderless table-striped small">
                          <thead>
                             <tr class="text-secondary">
                                <th class="pt-4 pb-3 px-6">First Name</th>
                                <th class="pt-4 pb-3 px-6">Last Name</th>
                                @foreach ($maingroups as $groups)
                                @if ($groups->id == $i->maingroup_id)
                                <th class="pt-4 pb-3 px-6">Type</th>
                                @endif
                                @endforeach
                                <th class="pt-4 pb-3 px-6">Email Address</th>
                                <th class="pt-4 pb-3 px-6">Invite Date</th>
                                <th class="pt-4 pb-3 px-6">Attempt Date</th>
                                <th class="pt-4 pb-3 px-6">Attempted</th>
                                <th class="pt-4 pb-3 px-6">Status</th>
                                <th class="pt-4 pb-3 px-6">Result</th>
                          </thead>
                          <tbody id="clientrows_{{ $i->clientid }}">
                             @foreach ($inv as $iv)
                             <tr class="text-secondary">
                                <td class="pt-4 pb-3 px-6"
                                   id="firstname{{ $iv->invitationid }}">
                                   {{ $iv->firstname }}
                                </td>
                                <td class="pt-4 pb-3 px-6"
                                   id="lastname{{ $iv->invitationid }}">
                                   {{ $iv->lastname }}
                                </td>
                                <td class="pt-4 pb-3 px-6"
                                   id="type{{ $iv->invitationid }}">
                                   {{ $iv->name }}
                                </td>
                                <td class="pt-4 pb-3 px-6"
                                   id="firstname{{ $iv->invitationid }}">
                                ...
                                @if ($iv->attempt == 'n')
                                <td class="pt-4 pb-3 px-6"
                                   id="firstname{{ $iv->invitationid }}">Not
                                   Attempted
                                </td>
                                @else
                                <td class="pt-4 pb-3 px-6"
                                ...
                                <td class="pt-4 pb-3 px-6"
                                   ...
                                   <a
                                      target="_blank"
                                      href="viewResultAdmin?id={{ $iv->invitationid }}">
                                      <svg xmlns="http://www.w3.org/2000/svg"
                                         width="18" height="18"
                                         fill="green" class="bi bi-eye"
                                         viewBox="0 0 16 16">
                                         <path
                             ...
                             @else
                             <td class="pt-4 pb-3 px-6"
                                id="firstname{{ $iv->invitationid }}">Not Completed
                             </td>
                             @endif
                             </tr>
                             @endforeach
...
<script>
  ...
  $("#actionnew").click(function() {
      ...
      if (maingroup == "") {
          Swal.fire('Please Enter the Select the Exam!', '', 'error')
          return
      }
      ...
      if (action == "Invite") {
          add(firstname, lastname, emailaddress, testinguser, maingroup)
      }
  ...
  function add(firstname, lastname, emailaddress, testinguser, maingroup) {
      $.ajaxSetup({
          headers: {
     ...
     <div class="accordion-item">\
     <h2 class="accordion-header" id="headingOne">\
     <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne' + data +
                  '" aria-expanded="false" aria-controls="collapseOne' + data + '"> &nbsp;' + firstname +
                  ' ' + lastname + '</button></h2>\
     ...
     <tbody id="clientrows_' + clientid + '"><tr class="text-secondary"><td class="pt-4 pb-3 px-6" id="firstname' +
                  data + '">' + firstname + '</td><td class="pt-4 pb-3 px-6" id="lastname' + data + '">' +
                  lastname + '</td><td class="pt-4 pb-3 px-6" id="firstname' + data + '">' +
                  emailaddress + '</td><td class="pt-4 pb-3 px-6" id="firstname' + data + '">' + date +
                  '</td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '"></td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">Not Attempted</td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">Not Started</td><td class="pt-4 pb-3 px-6" id="firstname' + data + '">Not Available</td></tr>\
  ...
  function inviteagain(id, maingroup_id) {
      $.ajaxSetup({
          headers: {
              'X-CSRF-TOKEN': $("meta[name='csrf_token']").attr('content')
          }
      ...
      $.ajax({
          ...
          data: {
              clientid: id,
              maingroupId: maingroup_id
          },
          success: function(data) {
            $("#inviteAgain").modal('toggle')
              var firstname = data[1]
              var lastname = data[2]
              var emailaddress = data[3]
              var groupName = data[4].name
              console.log(groupName)
              data = data[0]
              var d = new Date()
              var date = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate()
              $('#clientrows_' + id).append(
                  '<tr class="text-secondary"><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">' + firstname + '</td><td class="pt-4 pb-3 px-6" id="lastname' + data + '">' +
                  lastname + '</td><td class="pt-4 pb-3 px-6" id="lastname' + data + '">' +
                  groupName + '</td><td class="pt-4 pb-3 px-6" id="firstname' + data + '">' +
                  emailaddress + '</td><td class="pt-4 pb-3 px-6" id="firstname' + data + '">' +
                  date + '</td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '"></td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">Not Attempted</td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">Not Started</td><td class="pt-4 pb-3 px-6" id="firstname' + data +
                  '">Not Available</td></tr>')
  ...
  $("#searchme").keyup(function() {
     ...
     <div class="accordion-item">\
     <h2 class="accordion-header" id="headingOne">\
     <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne' + data[i]
                          .clientid + '" aria-expanded="false" aria-controls="collapseOne' + data[
                              i].clientid + '"> &nbsp;' + inv[0].firstname + " " + inv[0]
                          .lastname + '</button></h2>\
     ...
     <tbody id="clientrows_' + data[i].clientid + '">'
       ...
       <td class="pt-4 pb-3 px-6" id="firstname' + inv[j].invitationid + '">' + inv[j].attemptdate + '</td>'
                          ...
                          if (inv[j].attempt == "n")
                              txt += '<td class="pt-4 pb-3 px-6" id="firstname' + inv[j]
                              .invitationid + '">Not Attempted</td>'
                          else
                              txt += '<td class="pt-4 pb-3 px-6" id="firstname' + inv[j]
                              .invitationid + '">Attempted</td>'
  
                          txt += '<td class="pt-4 pb-3 px-6" id="firstname' + inv[j]
                              .invitationid + '">' + inv[j].stage + '</td>'
                          if (inv[j].stage == "Closed") {
                              txt += '<td class="pt-4 pb-3 px-6" id="firstname' + inv[j]
                                  .invitationid +
                                  '"><a target="_blank" href="viewResultAdmin?id=' + inv[j]
                                  .invitationid +
                                  '"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="green" class="bi bi-eye" viewBox="0 0 16 16"> <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/><path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z"/></svg></a></td>'
                          } else {
                              txt += '<td class="pt-4 pb-3 px-6" id="firstname' + inv[j]
                                  .invitationid + '">Not Completed</td>'
                          }
                          txt += '</tr>'
                      }
                      txt += '</tbody></table><br>'
